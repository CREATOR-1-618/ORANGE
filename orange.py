...
if __name__ == "__main__":
    ...
    with st.sidebar:
        selected = option_menu("OrangeViz",
    ...
    st.title("Welcome to my OrangeViz 📞")
    ...
    if selected == "Presentation":
        ...
        st.markdown("Le groupe Orange présent au niveau des 4 coins du monde propose depuis 1988 des services télécommunications adaptés aux habitudes de leurs clients. "
    ...
    if selected == "Transforming the Data":
        ...
        hermes = pd.read_csv('orange.csv')

        st.subheader('Orange data after transform')
        hermes= drop_col(hermes)
        st.markdown('Orange data après la suppression des colonnes')
        ...
        st.markdown(
            'Afin de réaliser une étude temporelle des likes et comments des posts, j ai rajouté les colones day, hour et month.')
    ...
    if selected == "Frequency":
        ...
        st.markdown(
            'Voici des statistiques sur le nombre de likes et comments par hour, day, weekday et month.')
    ...
    if selected == "Top 10":
        ...
        heatmap(hermes)
        st.markdown('Cette corrélation permet de mettre en évidence l activité de votre compte selon le jour de la semaine et les heures')

        st.title('Summary')
        st.markdown('Grâce aux outils de la data, que j ai employé tels que pandas, matplotlib, seaborn et plotly, j ai réussi'
                    ' à souligner les périodes où votre audience est la plus active ou encore quels sont les types de posts qui'
                    ' marchent le plus. Cette étude peut vous aider à choisir la meilleure approche envers votre client sur Instagram, dans les futurs'
                    ' campagnes. Merci pour votre attention ❤️. \n '
                    ' Mes contacts sur la barre de navigation  ⬅️pour prendre contact. 😉')
